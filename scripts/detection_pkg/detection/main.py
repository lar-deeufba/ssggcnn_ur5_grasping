...
class Detector:
    def __init__(self, model_path, model='ssd512', ctx='cpu', classes='grasp'):
...
det=Detector(path + "/scripts/detection_pkg/model2.params")
...
while not rospy.is_shutdown():
	...
	if cam.has_image > 0:
		...
		img=caixas.draw(im)
		cv2.circle(img,(int(caixas[0].x1),int(caixas[0].y1)), 2, (0,0,255), -1)
		cv2.circle(img,(int(caixas[0].x2),int(caixas[0].y2)), 2, (0,0,255), -1)
		cv2.imshow('image',img)
		cv2.waitKey(1)
		#print(caixas)
		point1= Point()
		point2= Point()
		point1.x=int(caixas[0].x1)
		point1.y=int(caixas[0].y1)
		point2.x=int(caixas[0].x2)
		point2.y=int(caixas[0].y2)
		#print(point1)
		#print(point2)
		pub1.publish(point1)
		pub2.publish(point2)
